...
## 프로젝트 간단 설명
Creator(=강사)가 강의를 개설하고, 학생이 수강 신청할 수 있는 REST API 서버입니다.
정원 초과 방지를 위한 동시성 처리와 수강 신청 상태 관리를 중점으로 구현했습니다.
...
### 1. 레포지토리 클론
```bash
git clone https://github.com/seoyeon-jung/liveclass-course-enrollment-system.git
cd liveclass-course-enrollment-system
```
...
수강 신청 상태: `PENDING` or `CONFIRMED` → `CANCELLED`
